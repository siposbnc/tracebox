...
/** Shared classes for a toolbar toggle/icon button. */
function toolCls(active: boolean, accent: 'sky' | 'amber' = 'sky'): string {
  return `rounded-lg border border-edge px-2.5 py-1.5 text-sm ${
    active
      ? accent === 'amber'
        ? 'bg-surface-3 text-amber-300'
...
function Divider() {
  return <span className="mx-0.5 h-5 w-px shrink-0 bg-edge" />;
}
  ...
  return (
    <div className="relative border-b border-edge bg-surface-1 px-3 pb-2 pt-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div className="relative" ref={historyRef}>
            ...
            {historyOpen && (
              <div className="absolute left-0 top-full z-30 mt-1 w-[420px] max-w-[90vw] rounded-lg border border-edge bg-surface-2 shadow-2xl">
                ...
                <div className="max-h-[60vh] overflow-y-auto p-2">
                  {saved.length > 0 && (
                    <div className="mb-2">
                      ...
                      {saved.map((s) => (
                        <div key={s.query} className="group flex items-center gap-2 rounded px-1 py-0.5 hover:bg-surface-3">
                          <button
          ...
          {/* query syntax help */}
          <button onClick={() => setHelpOpen((v) => !v)} className={toolCls(helpOpen)} title="Query syntax help">
            ?
          </button>
        </div>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium ${
            tail ? 'border-emerald-700 bg-emerald-950 text-emerald-300' : 'border-edge bg-surface-2 text-gray-400 hover:text-gray-100'
          }`}
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
        ...
        <button onClick={onToggleFacets} className={toolCls(facetsOpen)} title="Field breakdown">
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M3 6h18" /><path d="M3 12h12" /><path d="M3 18h6" />
          </svg>
        </button>
        <button onClick={onToggleClusters} className={toolCls(clustersOpen)} title="Log patterns (clustering)">
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <button
          ...
          className={`relative ${toolCls(watchOpen, 'amber')}`}
          title="Watch rules — alert on matching lines or rate thresholds while tailing"
        >
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {watchUnseen > 0 && (
            <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-500 px-1 text-[10px] font-semibold text-gray-950">
              {watchUnseen > 99 ? '99+' : watchUnseen}
            </span>
          )}
        </button>
        <BookmarksMenu
        ...
        {/* view modes */}
        <button onClick={onToggleGrouped} className={toolCls(grouped)} title="Group multi-line records (fold stack traces into one row)">
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M8 6h13M8 12h13M8 18h13" /><path d="M3 6v12" /><path d="M3 6h2M3 18h2" />
          </svg>
        </button>
        <button
          onClick={onToggleHighlight}
          className={toolCls(highlightMode, 'amber')}
          title={`Highlight matches in place instead of filtering${bindings.toggleHighlight ? ` (${formatChord(bindings.toggleHighlight)})` : ''}`}
        >
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <button
          ...
          className={toolCls(wrap)}
          title={`${wrap ? 'Wrapping long lines — click to truncate' : 'Truncating long lines — click to wrap'}${bindings.toggleWrap ? ` (${formatChord(bindings.toggleWrap)})` : ''}`}
        >
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <button
          ...
          title={tz === 'utc' ? 'Timestamps in UTC — click for local time' : 'Timestamps in local time — click for UTC'}
        >
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg border border-edge bg-surface-2 px-2.5 py-1.5 text-sm text-gray-400 hover:text-gray-100"
          title={order === 'asc' ? 'Oldest first — click to show newest first' : 'Newest first — click to show oldest first'}
        >
          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            {order === 'asc' ? (
              <>
        ...
        <div className="relative">
          ...
          {copyNote && (
            <div className="absolute right-0 top-full z-30 mt-1 whitespace-nowrap rounded-md border border-edge bg-surface-3 px-2.5 py-1 text-xs text-gray-200 shadow-lg">
              {copyNote}
            </div>
          )}
        </div>
